...
public class VisitorViewService {
    ...
    // 특정 날짜를 기준으로 7일 동안의 값을 찾아온다.
    public List<ViewReadCountStaticalDto> findReadCountStatDtoByDate(LocalDate date) {
        List<ReadCountStaticalDto> findValue = visitorViewRepository.findReadCountStaticalDtoByDate(date);

        return findValue.stream().map(readCountStaticalDto -> new ViewReadCountStaticalDto(
                readCountStaticalDto.getDate().format(DateTimeFormatter.ofPattern("yy.MM.dd")),
                readCountStaticalDto.getView(),
                readCountStaticalDto.getLoginView(),
                readCountStaticalDto.getUserJoin(),
                readCountStaticalDto.getWriteCreatedNumber())).collect(Collectors.toList());
    }
